...
namespace Keycloak.Net
{
    public partial class KeycloakClient
    {
        ...
        public async Task<bool> AddRealmRoleMappingsToUserAsync(string userId, string roleName, CancellationToken cancellationToken = default)
        {
            var role = await GetBaseUrl("deneme")
                   .AppendPathSegment($"/admin/realms/deneme/roles")
                   .GetJsonAsync<IEnumerable<Role>>(cancellationToken)
                   .ConfigureAwait(false);
            var rol = role.FirstOrDefault(x => x.Name == roleName);

            var response = await GetBaseUrl("deneme")
                .AppendPathSegment($"/admin/realms/deneme/users/{userId}/role-mappings/realm")
                .PostJsonAsync(rol, cancellationToken)
                .ConfigureAwait(false);
            return response.ResponseMessage.IsSuccessStatusCode;
        }
        public async Task<bool> AddRealmRoleMappingsToUserAsync2(string realm, string userId, IEnumerable<Role> roles, CancellationToken cancellationToken = default)
        {
            var response = await GetBaseUrl(realm)
        ...
        public async Task<IEnumerable<Role>> GetRealmRoleMappingsForUserAsync( string userId, CancellationToken cancellationToken = default) => await GetBaseUrl("")
            .AppendPathSegment($"/admin/realms/deneme/users/{userId}/role-mappings/realm")
            .GetJsonAsync<IEnumerable<Role>>(cancellationToken)
            .ConfigureAwait(false);
